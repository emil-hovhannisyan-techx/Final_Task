...
const floatingSearchButton = floatingContainer?.querySelector(".search-button");
...
function buildQueryFromFilters() {
  ...
  // 2. Genres: Now uses genre IDs and correctly gets checked values
  ...
  // 3. Show Me Filter: Now properly handles radio buttons
  ...
  // 4. Release Dates: Now properly reads date inputs
  ...
  // 5. Release Type Filters: Now properly reads release type checkboxes
  ...
  // 6. Sliders: Now correctly reads values and converts them to API parameters
...
allFormInputs.forEach((input) => {
  ...
  if (input.type === "text") input.addEventListener("keyup", updateButtonState);
});
...
function createMovieCard(movie) {
  ...
  movie.poster_path
    ...
    : (n.style.backgroundImage = 'url("/assets/main/default_background.svg")'),
    t.appendChild(n),
